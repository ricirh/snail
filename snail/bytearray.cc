...
namespace snail
{
    ...
    void ByteArray::writeFuint64(int64_t value)
    {
        if(m_endian != SNAIL_BYTE_ORDER)
        {
            value = byteswap(value);
    ...
    void ByteArray::writeUint64(int64_t value)
    {
        int8_t tmp[10];
        uint8_t i;
        while(value >= 0x80)
...
#undef XX
    ...
    bool ByteArray::writeToFile(const std::string &name) const
    {
        ...
        if(!ofs)
        {
            SNAIL_LOG_ERROR(g_logger) << "writeToFile name=" << name << " error, errno=" << errno
    ...
    bool ByteArray::readFromFile(const std::string &name)
    {
        ...
        if (!ifs)
        {
            SNAIL_LOG_ERROR(g_logger) << "readFromFile name=" << name << " error, errno=" << errno
                << " errstr=" << strerror(errno);
            return false;
        }
    ...
    {
        return m_endian == SNAIL_LITTLE_ENDIAN;
    }
