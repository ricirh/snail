...
namespace snail
{
    namespace http
    {
        ...
        {
            SNAIL_LOG_INFO(g_logger) << "HttpConnection::~HttpConnection";
        }
                                        ...
                                        , const std::string& body)
        {
            ...
            if(!uri)
            {
                return std::make_shared<HttpResult>((int)HttpResult::Error::INVALID_URL, nullptr, "invalid url");
            }
        ...
        {
            uint64_t now_ms = snail::GetCurrentMS();
            ...
            if(!ptr)
            {
                ...
                if(!addr)
                {
                    SNAIL_LOG_ERROR(g_logger) << "lookup any ip address failed, host=" << m_host;
                    return nullptr;
                }
                addr->setPort(m_port);
                Socket::ptr sock = Socket::CreateTCP(addr);
                if(!sock)
                {
                    SNAIL_LOG_ERROR(g_logger) << "create tcp socket failed, host=" << *addr;
                    return nullptr;
                }
                if(!sock->connect(addr))
                {
                    SNAIL_LOG_ERROR(g_logger) << "connect failed, host=" << *addr;
                    return nullptr;
                }
                                        ...
                                        , uint64_t timeout_ms)
        {
            ...
            if(!sock)
            {
                ...
                , nullptr, "pool invalid connection");
            }
            sock->setRecvTimeout(timeout_ms);
            int rt = conn->sendRequest(req);
            if(rt == 0)
        ...
        void HttpConnectionPool::RelasePtr(HttpConnection* ptr, HttpConnectionPool* pool)
        {
            ++ptr->m_request;
            if(!ptr->isConnected()
                || (ptr->m_createTime + pool->m_maxAliveTime >= snail::GetCurrentMS())
