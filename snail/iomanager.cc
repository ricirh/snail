...
namespace snail
{
    ...
    int IOManager::addEvent(int fd, Event event, std::function<void()> cb)
    {
        ...
        {
            SNAIL_LOG_ERROR(g_logger) << "addEvent assert fd = " << fd 
                << " event" << event
                << " fd_ctx.evnet = " << fd_ctx->m_events;
            SNAIL_ASSERT(!(fd_ctx->m_events & event));
        }
        ...
        if(rt)
        {
            SNAIL_LOG_ERROR(g_logger) << "epoll_ctl(" << m_epfd << ", "
        ...
        SNAIL_ASSERT(!event_ctx.scheduler 
                    && !event_ctx.fiber
                    && !event_ctx.cb);
        ...
        else
        {
            ...
            SNAIL_ASSERT(event_ctx.fiber->getState() == Fiber::EXEC);
        }
        return 0;
    }
    ...
    bool IOManager::cancelAll(int fd)
    {
        ...
        SNAIL_ASSERT(fd_ctx->m_events == 0);
        return true;
    }
    ...
    void IOManager::idle()
    {
        ...
        while(true)
        {
            ...
            {
                SNAIL_LOG_INFO(g_logger) << "name=" << getName() 
                    << " idle stopping exit";
                break;
